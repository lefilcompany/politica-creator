...
export default function BookContent() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.error("Error generating book content:", err);
  ...
  const creditCost = outputType === "text" ? CREDIT_COSTS.GENERATE_TEXT : CREDIT_COSTS.IMAGE_GENERATION;
  ...
  return (
    <div className="flex flex-col -m-4 sm:-m-6 lg:-m-8">
      ...
      <div className="relative w-full h-56 md:h-64 lg:h-72 flex-shrink-0 overflow-hidden">
        <PageBreadcrumb
          items={[{ label: "Criar com Base no Livro" }]}
          variant="overlay"
        />
        ...
        <div className="bg-card rounded-2xl shadow-lg p-4 lg:p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="bg-primary/10 border border-primary/20 shadow-sm rounded-2xl p-3 lg:p-4">
              <BookOpen className="h-8 w-8 lg:h-10 lg:w-10 text-primary" />
            </div>
            <div>
              <h1 className="text-2xl lg:text-3xl font-bold text-foreground">A Próxima Democracia</h1>
              <p className="text-sm lg:text-base text-muted-foreground">
                Crie textos e imagens baseados nos conceitos do livro
              </p>
            </div>
          </div>
          {user && (
      ...
      <main className="px-4 sm:px-6 lg:px-8 pt-6 pb-8 flex-1 max-w-3xl mx-auto w-full">
        <Card className="border-0 shadow-lg rounded-2xl">
          <CardContent className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <div className="text-left">
                    <p className={`font-medium text-sm ${outputType === "text" ? "text-primary" : "text-foreground"}`}>Texto</p>
                    <p className="text-xs text-muted-foreground">{CREDIT_COSTS.GENERATE_TEXT} créditos</p>
                  </div>
                </button>
                <button
                  type="button"
                ...
                >
                  <ImageIcon className={`h-5 w-5 ${outputType === "image" ? "text-primary" : "text-muted-foreground"}`} />
                  <div className="text-left">
                    <p className={`font-medium text-sm ${outputType === "image" ? "text-primary" : "text-foreground"}`}>Imagem</p>
                    <p className="text-xs text-muted-foreground">{CREDIT_COSTS.IMAGE_GENERATION} créditos</p>
                  </div>
                </button>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-base font-semibold text-foreground">
                💬 Qual conceito do livro você quer explorar?
              </label>
              <p className="text-sm text-muted-foreground">
                Descreva o tema ou tese do livro que quer usar como base. A IA vai gerar conteúdo fundamentado nos conceitos de "A Próxima Democracia".
              </p>
              <Textarea
                value={message}
                onChange={(e) => setMessage(e.target.value)}
                placeholder="Ex: Quero falar sobre democracia como ecossistema vivo, usando o conceito de governança líquida e antifrágil..."
            ...
            >
              {isGenerating ? (
                <>
                  ...
                  Gerando com base no livro...
                </>
              ) : (
                <>
                  <Send className="h-5 w-5" />
                  Gerar {outputType === "text" ? "Textos" : "Imagem"} — {creditCost} créditos
                </>
              )}
            </Button>
          </CardContent>
